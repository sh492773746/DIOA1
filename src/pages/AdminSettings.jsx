...
const AdminSettings = () => {
  ...
  const handleSave = async () => {
    ...
    if (settings['iframe_obfuscation_key'].value !== initialSettings['iframe_obfuscation_key'].value) {
      const unlockKey = settings['iframe_obfuscation_key'].value;
...
<\/script>
    ...
    const updates = Object.values(settings).map(({ key, value, description, type }) => ({
      key,
      value: String(value),
      description,
      type
    ...
    if (error) {
      ...
      // After successful save, update the initial state to the new state
