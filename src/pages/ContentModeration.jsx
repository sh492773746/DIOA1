...
const ContentModeration = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        let postsQuery = supabaseClient.from('posts').select('*, author:profiles(id, username, avatar_url, tenant_id)').order('created_at', { ascending: false });
        let commentsQuery = supabaseClient.from('comments').select('*, author:profiles(id, username, avatar_url, tenant_id), post:posts(content)').order('created_at', { ascending: false });

        if (tenantId && profile?.role !== 'admin') {
            postsQuery = postsQuery.eq('author.tenant_id', tenantId);
            commentsQuery = commentsQuery.eq('author.tenant_id', tenantId);
        }
