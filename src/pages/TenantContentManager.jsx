...
const TenantContentManager = () => {
    ...
    const fetchData = useCallback(async () => {
        if (!tenantId || !activePage || !supabaseClient) return;
        setLoading(true);
        ...
        let query = supabaseClient.from('page_content').select('*').eq('tenant_id', tenantId);
        ...
        setLoading(false);
    }, [toast, tenantId, activePage, supabaseClient]);
    
    useEffect(() => {
        if(activePage && tenantId && supabaseClient) {
            fetchData();
        }
    }, [fetchData, activePage, tenantId, supabaseClient]);
    ...
    const handleFormSubmit = async (values, itemId) => {
        ...
        let query;
        if(!itemId) {
            query = supabaseClient.from('page_content').insert([contentData]);
        } else {
            query = supabaseClient.from('page_content').update(contentData).eq('id', itemId);
        }
