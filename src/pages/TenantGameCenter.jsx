...
const TenantGameCenter = () => {
  ...
  useEffect(() => {
    const fetchGamesAndCategories = async () => {
      if (!tenantId) {
        setLoading(false);
        toast({ title: '错误', description: '未找到分站ID', variant: 'destructive' });
        return;
      }
      setLoading(true);
  ...
  return (
    <>
      <Helmet>
        <title>游戏中心 - 分站</title>
        <meta name="description" content="畅玩本站的各种趣味游戏" />
      </Helmet>
       <div className={cn("min-h-screen bg-gray-50", isDesktop && "flex")}>
          {isDesktop && !loading && gameCategories.length > 0 && <DesktopSidebar />}
          
          <main className="flex-1 overflow-y-auto">
            {!isDesktop && !loading && gameCategories.length > 0 && <MobileCategoryNav />}
