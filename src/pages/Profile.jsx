...
const Profile = () => {
  const { user, profile: authProfile, loading: authLoading, signOut, isSuperAdmin, isTenantAdmin, appTenantId, siteSettings } = useAuth();
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      if (isOwnProfile && isTenantAdmin) {
        const { data: tenantIdData, error: tenantIdError } = await supabase.rpc('get_user_tenant_id', { p_user_id: targetUserId });
        if (tenantIdError) {
  ...
  }, [userId, user, authLoading, isOwnProfile, authProfile, toast, isTenantAdmin]);
  ...
  const showAdminButton = isSuperAdmin && !appTenantId;
  const showTenantAdminButton = isTenantAdmin && appTenantId;
  
  return (
    <>
      <Helmet>
        <title>{profile.username}的个人主页 - {siteSettings.site_name}</title>
        <meta name="description" content={`查看${profile.username}在${siteSettings.site_name}上的个人资料和动态。`} />
      </Helmet>
      <motion.div 
      ...
      >
        <Card className="overflow-hidden shadow-lg border-none bg-gradient-to-br from-blue-50 to-purple-50">
          ...
          <CardContent className="p-6 space-y-6">
            ...
            <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center bg-white/60 p-3 rounded-lg">
                    <span className="text-gray-600 flex items-center"><Gift className="w-4 h-4 mr-2 text-red-500" />免费广告次数</span>
                    <span className="font-medium text-gray-800">{profile.free_ads_count || 0} 次</span>
                </div>
                 {profile.role === 'admin' && (
        ...
        <UserPostHistory userId={profile.id} />
