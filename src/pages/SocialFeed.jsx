...
const PinnedAdsSection = () => {
  const { isInitialized, supabaseClient, appTenantId } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchPinnedAds = async () => {
      ...
      const tenantIdToFetch = appTenantId ?? 0;

      const { data, error } = await fetchWithRetry(() => 
        supabaseClient.rpc('get_tenant_content', {
          p_page: 'social',
          p_section: 'pinned_ads',
          p_tenant_id: tenantIdToFetch
        })
      );
    ...
    fetchPinnedAds();
  }, [isInitialized, supabaseClient, appTenantId]);
...
const SocialFeed = () => {
  const { user, isInitialized, appTenantId, siteSettings, supabaseClient } = useAuth();
  ...
  const fetchPosts = useCallback(async () => {
    ...
    let query = supabaseClient
      ...
      .select(`
        *,
        author:profiles!left(*),
        likes(*, author:profiles(username, avatar_url)),
        comments(*, author:profiles!left(*)),
  ...
  }, [fetchPosts, isInitialized, supabaseClient]);
  ...
  return (
    <>
      <Helmet>
        <title>朋友圈 - {siteSettings.site_name}</title>
      ...
      {user && <FloatingCreatePostButton onPostCreated={handlePostCreated} tenantId={appTenantId || 0} />}
    </>
  );
};
