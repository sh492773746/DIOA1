...
const GameCenter = () => {
  ...
  const { user, appTenantId, isInitialized, siteSettings } = useAuth();
  ...
  const fetchGamesAndCategories = useCallback(async () => {
    ...
    if (appTenantId) {
        gameCardsQuery = gameCardsQuery.eq('tenant_id', appTenantId);
    } else {
    ...
    let gameCategoriesQuery = supabase
        .from('page_content')
    ...
    if(appTenantId) {
        gameCategoriesQuery = gameCategoriesQuery.eq('tenant_id', appTenantId);
    } else {
        gameCategoriesQuery = gameCategoriesQuery.is('tenant_id', null);
    }

    const { data: categoriesData, error: categoriesError } = await fetchWithRetry(() => gameCategoriesQuery);
    ...
    setLoading(false);
  }, [toast, isInitialized, appTenantId]);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={`畅玩${siteSettings.site_name}的各种趣味游戏`} />
      </Helmet>
