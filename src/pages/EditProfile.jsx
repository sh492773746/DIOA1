...
const EditProfile = () => {
  ...
  const canChangeUsername = useMemo(() => {
    if (!authProfile?.last_username_update) {
      return true;
    }
    const lastUpdate = new Date(authProfile.last_username_update);
    const nextAvailableDate = add(lastUpdate, { days: 7 });
    return new Date() > nextAvailableDate;
  }, [authProfile]);
  
  useEffect(() => {
      if (!canChangeUsername && authProfile?.last_username_update) {
  ...
  }, [canChangeUsername, authProfile?.last_username_update]);
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    let finalAvatarUrl = authProfile.avatar_url;
    
    const usernameChanged = username.trim() !== authProfile.username;
  ...
  const isSaveDisabled = isSaving || 
    (!newAvatarFile && (username.trim() === authProfile?.username || !username.trim()));


  return (
    <>
      <Helmet>
        <title>{String('编辑个人资料 - SocialSphere')}</title>
