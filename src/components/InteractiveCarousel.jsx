...
const InteractiveCarousel = () => {
  ...
  const fetchCarouselData = useCallback(async () => {
    ...
    let settingsQuery = supabase.from('app_settings').select('value').eq('key', 'carousel_overlay_opacity').eq('tenant_id', tenantIdToQuery);
    const { data: settingsData } = await fetchWithRetry(() => settingsQuery);

    if (settingsData && settingsData.length > 0) {
      setDefaultOpacity(Number(settingsData[0].value) || 10);
    } else {
      const { data: mainSettingsData } = await fetchWithRetry(() => supabase.from('app_settings').select('value').eq('key', 'carousel_overlay_opacity').eq('tenant_id', 0));
      setDefaultOpacity(mainSettingsData && mainSettingsData.length > 0 ? Number(mainSettingsData[0].value) : 10);
    }
    ...
    const { data: contentData, error: contentError } = await fetchWithRetry(() => contentQuery);
  ...
  useEffect(() => {
    ...
    const channelName = `page_content_carousel_channel_${appTenantId || '0'}`;
    const channel = supabase.channel(channelName)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'page_content', filter: `section=eq.carousel` },
        (payload) => {
            const tenantMatch = (appTenantId ?? 0) === (payload.new.tenant_id ?? 0);
            if (tenantMatch) {
                fetchCarouselData();
            }
        }
