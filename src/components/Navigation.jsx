...
const Navigation = () => {
  const { profile, signOut, user, appTenantId, isInitialized, supabaseClient } = useAuth();
  ...
  const fetchUnreadCount = useCallback(async () => {
      if (!user || !supabaseClient) return;
      const { count, error } = await fetchWithRetry(() => supabaseClient
          .from('notifications')
  ...
  }, [appTenantId, supabaseClient]);

  useEffect(() => {
    if (!isInitialized || !supabaseClient) return;
    fetchLogo();

    const channel = supabaseClient.channel(`site_settings_logo_channel_${appTenantId || 'main'}`)
  ...
  }, [fetchLogo, isInitialized, appTenantId, supabaseClient]);
  ...
  }, [user, fetchNotifications, fetchUnreadCount, supabaseClient]);
          ...
          <div className="flex items-center space-x-3">
            {user ? (
              <>
                ...
                <div className="hidden md:flex items-center space-x-2 bg-muted rounded-full px-3 py-1">
             ...
             <div className="md:hidden">
              <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-muted-foreground hover:bg-muted">
