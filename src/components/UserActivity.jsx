...
import { MessageSquare, FileText, Frown } from 'lucide-react';
...
const UserActivity = ({ userId }) => {
    ...
    const fetchActivity = useCallback(async () => {
        ...
        if (postsResponse.error) console.error("Error fetching posts:", postsResponse.error.message);
        if (commentsResponse.error) console.error("Error fetching comments:", commentsResponse.error.message);
        
        setPosts(postsResponse.data ? postsResponse.data.map(p => ({...p, likes_count: p.likes_count[0]?.count || 0})) : []);
