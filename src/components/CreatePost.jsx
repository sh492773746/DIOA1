...
const CreatePost = ({ isOpen, setIsOpen, onPostCreated, tenantId }) => {
    ...
    const [useFreeAd, setUseFreeAd] = useState(true);
    ...
    useEffect(() => {
        const fetchCosts = async () => {
            ...
            const globalQuery = query.eq('tenant_id', 0);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const { data, error } = await supabase.rpc('create_post_and_deduct_points', {
                ...
                p_use_free_ad: isAd && useFreeAd && profile?.free_ads_count > 0,
            });
            ...
            const cost = getPostCost();
            const description = cost > 0 ? `消耗了 ${cost} 积分。` : `使用了一次免费额度。`;

            toast({ title: "🎉 发布成功！", description: `你的动态正在审核中。${description}` });
            setIsOpen(false);
            resetForm();
    ...
    const getPostCost = () => {
        if (isAd) {
            return useFreeAd && profile?.free_ads_count > 0 ? 0 : costs.ad;
        }
        return costs.social;
    };
            ...
            <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                    ...
                    {imagePreviews.length > 0 && (
                     ...
                     {isAd && profile?.free_ads_count > 0 && (
                        <div className="flex items-center space-x-2 bg-primary/10 p-2 rounded-md">
                            <Checkbox
                                id="use-free-ad"
                                checked={useFreeAd}
                                onCheckedChange={setUseFreeAd}
                            />
                            <Label htmlFor="use-free-ad" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                                使用免费广告额度 (剩余 {profile.free_ads_count} 次)
                            </Label>
                        </div>
                     )}
                     <p className="text-sm text-muted-foreground text-center">
                        本次发布将消耗 <span className="font-bold text-primary">{getPostCost()}</span> 积分。
                    </p>
                </div>
                <DialogFooter>
                    <Button variant="ghost" onClick={() => setIsOpen(false)}>取消</Button>
                    <Button onClick={handleSubmit} disabled={isPosting} className="bg-primary hover:bg-primary/90 text-primary-foreground">
                        {isPosting ? (
                            <>
